...
function HuskyTable({dispatch,huskyTableM, }) {
    ...
    const edit = (newDataSource) => {
        dispatch({ type: 'LXhuskyTableM/updateState', payload: { dataSource: newDataSource,dataSourceBuffer:newDataSource,isEditLock: true}});
    ...
    const checkConfim = (newDataSource) => {
        dispatch({ type: 'LXhuskyTableM/updateState', payload: { dataSource: newDataSource , dataSourceBuffer:newDataSource,isEditLock: false,isNewItem: false} });
    };
    ...
    const checkCancle = (flag,newDataSource) => {
        ...
        flag && dispatch({ type: 'LXhuskyTableM/updateState', payload: { dataSource: newDataSource, dataSourceBuffer: newDataSource, isEditLock: false, } });

        !flag && dispatch({ type: 'LXhuskyTableM/updateState', payload: { isNewItem: false, isEditLock: false } });
    ...
    const onCellChange = (dataSourceBuffer) => {
        dispatch({ type: 'LXhuskyTableM/updateState', payload: { dataSourceBuffer: dataSourceBuffer,} });
    }
    ...
    const handleDelete = (dataSourceBuffer) => {        
        dispatch({ type: 'LXhuskyTableM/updateState', payload: { dataSourceBuffer: dataSourceBuffer, dataSource: dataSourceBuffer, isEditLock: false } });
    }
    ...
    const handleAdd = (newDataSource) => {
            dispatch(
                {
                    type: 'LXhuskyTableM/updateState',
                    payload: {
    ...
    const searchClick = (newDataSource) => {
        dispatch({ type: 'LXhuskyTableM/updateState', payload: { dataSourceBuffer: newDataSource,} });
    }
    

    const onChangeTable=(current)=>{
        dispatch({ type: 'LXhuskyTableM/updateState', payload: { paginationCurrent: current,} })
    }
    const emitEmptyhandle = () => {
        dispatch({type: 'LXhuskyTableM/updateState',payload: {searchValue: '' ,} })
    }

    const onChangeSearchValue=(value)=>{
        dispatch({type: 'LXhuskyTableM/updateState',payload: {searchValue: value ,}});
    }

    // const isCanOrderChange=(prams) => {
    //     dispatch({type: 'LXhuskyTableM/updateState',payload: {isCanOrder: prams,}});        
    // }   
...
export default connect(({LXhuskyTableM}) => ({huskyTableM:LXhuskyTableM}))(HuskyTable);
