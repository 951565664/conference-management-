...
function HuskyTable({dispatch,huskyTableM, }) {
    ...
    const edit = (newDataSource) => {
        dispatch({ type: 'KQhuskyTableM/updateState', payload: { dataSource: newDataSource,dataSourceBuffer:newDataSource,isEditLock: true}});
    ...
    const checkConfim = (newDataSource) => {
        dispatch({ type: 'KQhuskyTableM/updateState', payload: { dataSource: newDataSource , dataSourceBuffer:newDataSource,isEditLock: false,isNewItem: false} });
    };
    ...
    const checkCancle = (flag,newDataSource) => {
        ...
        flag && dispatch({ type: 'KQhuskyTableM/updateState', payload: { dataSource: newDataSource, dataSourceBuffer: newDataSource, isEditLock: false, } });

        !flag && dispatch({ type: 'KQhuskyTableM/updateState', payload: { isNewItem: false, isEditLock: false } });
    ...
    const onCellChange = (dataSourceBuffer) => {
        dispatch({ type: 'KQhuskyTableM/updateState', payload: { dataSourceBuffer: dataSourceBuffer,} });
    }
    ...
    const handleDelete = (dataSourceBuffer) => {        
        dispatch({ type: 'KQhuskyTableM/updateState', payload: { dataSourceBuffer: dataSourceBuffer, dataSource: dataSourceBuffer, isEditLock: false } });
    }
    ...
    const handleAdd = (newDataSource) => {
            dispatch(
                {
                    type: 'KQhuskyTableM/updateState',
                    payload: {
    ...
    const searchClick = (newDataSource) => {
        dispatch({ type: 'KQhuskyTableM/updateState', payload: { dataSourceBuffer: newDataSource,} });
    }
    

    const onChangeTable=(current)=>{
        dispatch({ type: 'KQhuskyTableM/updateState', payload: { paginationCurrent: current,} })
    }
    const emitEmptyhandle = () => {
        dispatch({type: 'KQhuskyTableM/updateState',payload: {searchValue: '' ,} })
    }

    const onChangeSearchValue=(value)=>{
        dispatch({type: 'KQhuskyTableM/updateState',payload: {searchValue: value ,}});
    }

    // const isCanOrderChange=(prams) => {
    //     dispatch({type: 'KQhuskyTableM/updateState',payload: {isCanOrder: prams,}});        
    // }   
...
export default connect(({KQhuskyTableM}) => ({huskyTableM:KQhuskyTableM}))(HuskyTable);
