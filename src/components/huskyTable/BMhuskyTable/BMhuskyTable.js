...
function HuskyTable({dispatch,huskyTableM, }) {
    ...
    const edit = (newDataSource) => {
        dispatch({ type: 'BMhuskyTableM/updateState', payload: { dataSource: newDataSource,dataSourceBuffer:newDataSource,isEditLock: true}});
    ...
    const checkConfim = (newDataSource) => {
        dispatch({ type: 'BMhuskyTableM/updateState', payload: { dataSource: newDataSource , dataSourceBuffer:newDataSource,isEditLock: false,isNewItem: false} });
    };
    ...
    const checkCancle = (flag,newDataSource) => {
        ...
        flag && dispatch({ type: 'BMhuskyTableM/updateState', payload: { dataSource: newDataSource, dataSourceBuffer: newDataSource, isEditLock: false, } });

        !flag && dispatch({ type: 'BMhuskyTableM/updateState', payload: { isNewItem: false, isEditLock: false } });
    ...
    const onCellChange = (dataSourceBuffer) => {
        dispatch({ type: 'BMhuskyTableM/updateState', payload: { dataSourceBuffer: dataSourceBuffer,} });
    }
    ...
    const handleDelete = (dataSourceBuffer) => {        
        dispatch({ type: 'BMhuskyTableM/updateState', payload: { dataSourceBuffer: dataSourceBuffer, dataSource: dataSourceBuffer, isEditLock: false } });
    }
    ...
    const handleAdd = (newDataSource) => {
            dispatch(
                {
                    type: 'BMhuskyTableM/updateState',
                    payload: {
    ...
    const searchClick = (newDataSource) => {
        dispatch({ type: 'BMhuskyTableM/updateState', payload: { dataSourceBuffer: newDataSource,} });
    }
    

    const onChangeTable=(current)=>{
        dispatch({ type: 'BMhuskyTableM/updateState', payload: { paginationCurrent: current,} })
    }
    const emitEmptyhandle = () => {
        dispatch({type: 'BMhuskyTableM/updateState',payload: {searchValue: '' ,} })
    }

    const onChangeSearchValue=(value)=>{
        dispatch({type: 'BMhuskyTableM/updateState',payload: {searchValue: value ,}});
    }

    // const isCanOrderChange=(prams) => {
    //     dispatch({type: 'BMhuskyTableM/updateState',payload: {isCanOrder: prams,}});        
    // }   
...
export default connect(({BMhuskyTableM}) => ({huskyTableM:BMhuskyTableM}))(HuskyTable);
