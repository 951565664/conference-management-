...
function HuskyTable({dispatch,huskyTableM, }) {
    ...
    const edit = (newDataSource) => {
        dispatch({ type: 'SBhuskyTableM/updateState', payload: { dataSource: newDataSource,dataSourceBuffer:newDataSource,isEditLock: true}});
    ...
    const checkConfim = (newDataSource) => {
        dispatch({ type: 'SBhuskyTableM/updateState', payload: { dataSource: newDataSource , dataSourceBuffer:newDataSource,isEditLock: false,isNewItem: false} });
    };
    ...
    const checkCancle = (flag,newDataSource) => {
        ...
        flag && dispatch({ type: 'SBhuskyTableM/updateState', payload: { dataSource: newDataSource, dataSourceBuffer: newDataSource, isEditLock: false, } });

        !flag && dispatch({ type: 'SBhuskyTableM/updateState', payload: { isNewItem: false, isEditLock: false } });
    ...
    const onCellChange = (dataSourceBuffer) => {
        dispatch({ type: 'SBhuskyTableM/updateState', payload: { dataSourceBuffer: dataSourceBuffer,} });
    }
    ...
    const handleDelete = (dataSourceBuffer) => {        
        dispatch({ type: 'SBhuskyTableM/updateState', payload: { dataSourceBuffer: dataSourceBuffer, dataSource: dataSourceBuffer, isEditLock: false } });
    }
    ...
    const handleAdd = (newDataSource) => {
            dispatch(
                {
                    type: 'SBhuskyTableM/updateState',
                    payload: {
    ...
    const searchClick = (newDataSource) => {
        dispatch({ type: 'SBhuskyTableM/updateState', payload: { dataSourceBuffer: newDataSource,} });
    }
    

    const onChangeTable=(current)=>{
        dispatch({ type: 'SBhuskyTableM/updateState', payload: { paginationCurrent: current,} })
    }
    const emitEmptyhandle = () => {
        dispatch({type: 'SBhuskyTableM/updateState',payload: {searchValue: '' ,} })
    }

    const onChangeSearchValue=(value)=>{
        dispatch({type: 'SBhuskyTableM/updateState',payload: {searchValue: value ,}});
    }

    // const isCanOrderChange=(prams) => {
    //     dispatch({type: 'SBhuskyTableM/updateState',payload: {isCanOrder: prams,}});        
    // }   
...
export default connect(({SBhuskyTableM}) => ({huskyTableM:SBhuskyTableM}))(HuskyTable);
