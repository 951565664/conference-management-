...
function HuskyTable({dispatch,huskyTableM, }) {
    ...
    const edit = (newDataSource) => {
        dispatch({ type: 'JLhuskyTableM/updateState', payload: { dataSource: newDataSource,dataSourceBuffer:newDataSource,isEditLock: true}});
    ...
    const checkConfim = (newDataSource) => {
        dispatch({ type: 'JLhuskyTableM/updateState', payload: { dataSource: newDataSource , dataSourceBuffer:newDataSource,isEditLock: false,isNewItem: false} });
    };
    ...
    const checkCancle = (flag,newDataSource) => {
        ...
        flag && dispatch({ type: 'JLhuskyTableM/updateState', payload: { dataSource: newDataSource, dataSourceBuffer: newDataSource, isEditLock: false, } });

        !flag && dispatch({ type: 'JLhuskyTableM/updateState', payload: { isNewItem: false, isEditLock: false } });
    ...
    const onCellChange = (dataSourceBuffer) => {
        dispatch({ type: 'JLhuskyTableM/updateState', payload: { dataSourceBuffer: dataSourceBuffer,} });
    }
    ...
    const handleDelete = (dataSourceBuffer) => {        
        dispatch({ type: 'JLhuskyTableM/updateState', payload: { dataSourceBuffer: dataSourceBuffer, dataSource: dataSourceBuffer, isEditLock: false } });
    }
    ...
    const handleAdd = (newDataSource) => {
            dispatch(
                {
                    type: 'JLhuskyTableM/updateState',
                    payload: {
    ...
    const searchClick = (newDataSource) => {
        dispatch({ type: 'JLhuskyTableM/updateState', payload: { dataSourceBuffer: newDataSource,} });
    }
    

    const onChangeTable=(current)=>{
        dispatch({ type: 'JLhuskyTableM/updateState', payload: { paginationCurrent: current,} })
    }
    const emitEmptyhandle = () => {
        dispatch({type: 'JLhuskyTableM/updateState',payload: {searchValue: '' ,} })
    }

    const onChangeSearchValue=(value)=>{
        dispatch({type: 'JLhuskyTableM/updateState',payload: {searchValue: value ,}});
    }

    // const isCanOrderChange=(prams) => {
    //     dispatch({type: 'JLhuskyTableM/updateState',payload: {isCanOrder: prams,}});        
    // }   
...
export default connect(({JLhuskyTableM}) => ({huskyTableM:JLhuskyTableM}))(HuskyTable);
